function WycaAPI(options){
	...
	// Top
		...
		CAPTURE_SEGMENT				: 0xE164,
		CAPTURE_SEGMENT_CANCEL		: 0xE167,
		CAPTURE_SEGMENT_PAUSE		: 0xE16E,
		RESTART_BROWSER			: 0xE178,
	...
	this.EventCode = {
		// Topics
		CAPTURE_SEGMENT_LOG			: 0xE003,
		CAPTURE_SEGMENT_FEEDBACK	: 0xE065,
		CAPTURE_SEGMENT_RESULT	: 0xE066,
				...
				else if(data.type === 'wyca_api')
				{
					...
					if (_this.options.id_robot != 'not_robot')
					{
							...
							json.O == _this.CommandCode.SUBSCRIBE_ON_CHANGE)
						{
							...
							{
								/********** Actions *********/
								case _this.EventCode.CAPTURE_SEGMENT_FEEDBACK: 
								case _this.EventCode.CAPTURE_SEGMENT_RESULT:
						...
						if (json.O == _this.CommandCode.CAPTURE_SEGMENT)
						{
							_this.subscribeOnWebrtc[_this.EventCode.CAPTURE_SEGMENT_FEEDBACK] = 'subscribed';
							_this.subscribeOnWebrtc[_this.EventCode.CAPTURE_SEGMENT_RESULT] = 'subscribed';
						}
	...
	this.ProcessMessage = function(msg_raw)
	{
		...
		if (msg.O != undefined)
		{
			...
			switch(msg.O)
			{
				...
				case this.CommandCode.SUBSCRIBE_ON_CHANGE:
					...
					{
						/********** Topics *********/
							case this.EventCode.CAPTURE_SEGMENT_LOG:
								if (_this.options.onCaptureLog != undefined) { _this.options.onCaptureLog(msg.D.D); }
		...
		if (msg.E != undefined) // False pour test retour subscribe
		{
			...
			{
				/********** Actions *********/
				case this.EventCode.CAPTURE_SEGMENT_FEEDBACK:
					if (_this.options.onCaptureSegmentbFeedback != undefined) { delete msg.E; _this.options.onCaptureSegmentFeedback(msg); }
					break;
				case this.EventCode.CAPTURE_SEGMENT_RESULT:
					if (_this.options.onCaptureSegmentResult != undefined) { delete msg.E; _this.options.onCaptureSegmentResult(msg); }
					break;
				...
				/********** Topics *********/
				case this.EventCode.CAPTURE_Segment_LOG:
					if (_this.options.onCaptureLog != undefined) { _this.options.onCaptureLog(msg.D); }
				case this.EventCode.BATTERY_STATE:
	...
	{
		if (_this.options.onCaptureLog != undefined) { var n=_this.EventCode.CAPTURE_SEGMENT_LOG; var subscribe = { "O": _this.CommandCode.SUBSCRIBE_ON_CHANGE, "P": n}; _this.traxSend(JSON.stringify(subscribe)); }
	...
	this.on = function (event_name, callback)
	{
		...
		switch (event_name)
		{
			case 'onCaptureLog': ev_code = _this.EventCode.CAPTURE_SEGMENT_LOG; break;
		...
		if (ev_code == 0 && _this.options.id_robot == 'not_robot')
		{
			...
			{
				/********** Actions *********/
				case 'onCaptureSegmentFeedback': ev_code = _this.EventCode.CAPTURE_SEGMENT_FEEDBACK; break;
				case 'onCaptureSegmentResult': ev_code = _this.EventCode.CAPTURE_SEGMENT_RESULT; break;
	...
	this.off = function (event_name)
	{
		...
		switch (event_name)
		{
			case 'onCaptureLog': ev_code = _this.CommandCode.CAPTURE_SEGMENT_LOG; break;
			case 'onBatteryState': ev_code = _this.EventCode.BATTERY_STATE; break;
			case 'onTimeRemainingToEmptyMin': ev_code = _this.EventCode.BMS_TIME_REMAINING_TO_EMPTY; break;
			case 'onTimeRemainingToFullMin': ev_code = _this.EventCode.BMS_TIME_REMAINING_TO_FULL; break;
			case 'onDiagnosticAlert': ev_code = _this.EventCode.DIAGNOSTIC_ALERT; break;
	...
	this.RestartBrowser = function(in_fullscreen, callback){
		if (callback != undefined)
			this.callbacks[_this.CommandCode.RESTART_BROWSER] = callback;
		var action = {
			"O": _this.CommandCode.RESTART_BROWSER,
			"P": in_fullscreen
		};
		_this.wycaSend(JSON.stringify(action));
	}
	this.PauseCaptureSegment = function(in_pause, callback){
		if (callback != undefined)
			this.callbacks[_this.CommandCode.CAPTURE_SEGMENT_PAUSE] = callback;
		var action = {
			"O": _this.CommandCode.CAPTURE_SEGMENT_PAUSE,
			"P": in_pause
		};
		_this.wycaSend(JSON.stringify(action));
	}
	...
	this.StartWebcam = function(callback){
		if (callback != undefined)
			this.callbacks[_this.CommandCode.START_WEBCAM] = callback;
		var action = {
			"O": _this.CommandCode.START_WEBCAM
		};
		_this.wycaSend(JSON.stringify(action));
	}
	this.StopWebcam = function(callback){
		if (callback != undefined)
			this.callbacks[_this.CommandCode.STOP_WEBCAM] = callback;
		var action = {
			"O": _this.CommandCode.STOP_WEBCAM
		};
		_this.wycaSend(JSON.stringify(action));
	}
	this.SetWebcam = function(camera_name, callback){
		if (callback != undefined)
			this.callbacks[_this.CommandCode.SET_WEBCAM] = callback;
		var action = {
			"O": _this.CommandCode.SET_WEBCAM,
			"P": camera_name
		};
		_this.wycaSend(JSON.stringify(action));
	}
	...
	/* Actions */
	this.CaptureSegment = function(segment_id, start_x, start_y, end_x, end_y, step_distance, callback){
		if (callback != undefined)
			this.callbacks[_this.CommandCode.CAPTURE_SEGMENT] = callback;
		var action = {
			"O": _this.CommandCode.CAPTURE_SEGMENT,
			"P": {
				"ID":segment_id,
				"SX":start_x,
				"SY":start_y,
	...
	this.CaptureSegmentCancel = function(callback){
		if (callback != undefined)
			this.callbacks[_this.CommandCode.CAPTURE_SEGMENT_CANCEL] = callback;
		var action = {
			"O": _this.CommandCode.CAPTURE_SEGMENT_CANCEL,
		};
		_this.wycaSend(JSON.stringify(action));
	}
}
