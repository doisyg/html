...
function ByStepTraceArea(indexArea)
{
	...
	if (points.length > 1)
	{
		...
		if(!is_moving){
			...
			let svg_polygon = $('#install_by_step_edit_map_area_'+area.id_area);
			...
			$.each(points, function( indexPoint, point ) {
				let svg_line = $('#install_by_step_edit_map_svg #install_by_step_edit_map_area_trait_'+area.id_area+'_'+indexPoint);
				let svg_point = $('#install_by_step_edit_map_svg #install_by_step_edit_map_area_'+area.id_area+'_'+indexPoint);
...
function ByStepTraceRobot(robot_x, robot_y, robot_theta)
{
	...
	else
	{
		if(robot_x == robot_y && robot_y == robot_theta && robot_x == 0 ){
			...
			$('#install_by_step_edit_map_robot_circle').show();
			$('#install_by_step_edit_map_robot_sens').show();
		}
	}
}
