...
{
	$('#install_normal_edit_map .burger_menu').addClass('updatingMap');
	wycaApi.GetCurrentMapComplete(function(data) {
		if (data.A == wycaApi.AnswerCode.NO_ERROR)
		{
			...
			setTimeout(function(){
				...
				$('#install_normal_edit_map .modalReloadMap').modal('hide');
				$('#install_normal_edit_map .modalReloadMap .btn').removeClass('disabled');
				$('#install_normal_edit_map .modalReloadMap .install_normal_edit_map_modalReloadMap_loading').hide();
				
				$('#install_normal_edit_map .modalConfirmNoReloadMap').modal('hide');
...
$(document).ready(function(e) {
	...
	*/
	...
	$(document).on('touchend', '#install_normal_edit_map_svg .landmark_elem', function(e) {
		$('#install_normal_edit_map_zoom_popup').hide();
		if (timerNormalLongPress != null)
		{
			clearTimeout(timerNormalLongPress);
	...
	/*
	$(document).on('touchstart', '#install_normal_edit_map_svg .landmark_elem', function(e) {
		if (timerNormalLongPress != null)
		{
			clearTimeout(timerNormalLongPress);
			timerNormalLongPress = null;
	...
	*/
	...
	$(document).on('touchmove', '#install_normal_edit_map_svg .landmark_elem', function(e) {
    	//NormalHideMenus();
		if (timerNormalLongPress != null)
		{
			clearTimeout(timerNormalLongPress);
...
function NormalHideMenus()
{
	...
	$('#install_normal_edit_map_menu_landmark li').hide();
...
function NormalLongPressLandmark()
{
	timerNormalLongPress = null;
...
$(document).ready(function(e) {
	...
	$('#install_normal_setup_trinary_zone_zoom_click').mousedown(function(e) {
       e.preventDefault();
	   downOnZoomClickTrinary = true;
    });
    
	$('#install_normal_setup_trinary_zone_zoom_click').mousemove(function(e) {
       e.preventDefault();
	   if (downOnZoomClickTrinary)
	   {
			w = $('#install_normal_setup_trinary_zoom_carte').width();
			h = $('#install_normal_setup_trinary_zoom_carte').height();
			
			wZoom = $('#install_normal_setup_trinary_zone_zoom').width();
			hZoom = $('#install_normal_setup_trinary_zone_zoom').height();
			...
			window.panZoomWycaTrinary.pan({'x':x, 'y':y});
	   }
    });
	
	$('#install_normal_setup_trinary_zone_zoom_click').click(function(e) {
		e.preventDefault();
	...
	$('#install_normal_setup_trinary_zone_zoom_click').on('touchstart', function(e) {
       e.preventDefault();
	   downOnZoomClickTrinary = true;
		...
		r = document.getElementById("install_normal_setup_trinary_zoom_carte_container").getBoundingClientRect();
	...
	$('#install_normal_setup_trinary_zone_zoom_click').on('touchend', function(e) {
       e.preventDefault();
	   downOnZoomClickTrinary = false;
    });
	
	$('#install_normal_setup_trinary_zone_zoom_click').on('touchmove', function(e) {
       e.preventDefault();
	   if (downOnZoomClickTrinary)
	   {
		    w = $('#install_normal_setup_trinary_zoom_carte').width();
	...
	$('#install_normal_setup_trinary_svg').on('touchend', function(e) {
		resetPanTrinary = true;
	});
	$('#install_normal_setup_trinary_svg').on('touchstart', function(e) {
		resetPanTrinary = true;
	});
})
...
function InitTrinaryNormalMap()
{
	...
	window.panZoomWycaTrinary = svgPanZoom('#install_normal_setup_trinary_svg', {
	  zoomEnabled: true
	, controlIconsEnabled: false
	, maxZoom: 20
	, fit: 1
	...
	svgTrinaryNormal = document.querySelector('#install_normal_setup_trinary_svg .svg-pan-zoom_viewport');
...
function TrinaryGetZoomNormal()
{
	...
	if (transformMatrix == undefined && typeof(window.panZoomWycaTrinary) != 'undefined' )
	 	return  current_map_obj.ros_width / $('#install_normal_setup_trinary_svg').width() / window.panZoomWycaTrinary.getZoom()
...
function TrinaryRefreshZoomViewNormal()
{
	...
	if (false && pImg.left > 0)
		...
		lNew = -(pImg.left*zoom) / current_map_obj.ros_width * wZoom;
	if (false && pImg.top > 0)
		tNew = 0;
	else
		tNew = -(pImg.top*zoom) / current_map_obj.ros_width * wZoom;
	
	hNew = $('#install_normal_setup_trinary_svg').height() * zoom  / current_map_obj.ros_width * wZoom;
	wNew = $('#install_normal_setup_trinary_svg').width() * zoom  / current_map_obj.ros_width * wZoom;
	
	//if (tNew + hNew > hZoom) hNew = hZoom - tNew;
	//if (lNew + wNew > wZoom) wNew = wZoom - lNew;
		
	$('#install_normal_setup_trinary_zone_zoom').width(wNew);
	$('#install_normal_setup_trinary_zone_zoom').height(hNew);
				
	$('#install_normal_setup_trinary_zone_zoom').css('top', tNew - 1);
	$('#install_normal_setup_trinary_zone_zoom').css('left', lNew - 1);
