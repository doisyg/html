...
$(document).ready(function(e) {
	...
	$('#wyca_by_step_mapping_zone_zoom_click').mousedown(function(e) {
       e.preventDefault();
	   downOnZoomClickTrinary = true;
    });
    
	$('#wyca_by_step_mapping_zone_zoom_click').mousemove(function(e) {
       e.preventDefault();
	   if (downOnZoomClickTrinary)
	   {
			w = $('#wyca_by_step_mapping_zoom_carte').width();
			h = $('#wyca_by_step_mapping_zoom_carte').height();
			
			wZoom = $('#wyca_by_step_mapping_zone_zoom').width();
			hZoom = $('#wyca_by_step_mapping_zone_zoom').height();
			
			x = e.offsetX - wZoom / 2;
			y = e.offsetY - hZoom / 2;
					
			//zoom = $('#wyca_by_step_mapping_image').attr('width') / $('#wyca_by_step_mapping_svg').width() / window.panZoomTrinary.getZoom();
			zoom = TrinaryGetZoom();
			
			largeur_img = $('#wyca_by_step_mapping_image').attr('width') / zoom
	...
	$('#wyca_by_step_mapping_zone_zoom_click').click(function(e) {
		e.preventDefault();
	...
	$('#wyca_by_step_mapping_zone_zoom_click').on('touchstart', function(e) {
       e.preventDefault();
	   downOnZoomClickTrinary = true;
		...
		r = document.getElementById("wyca_by_step_mapping_zoom_carte_container").getBoundingClientRect();
	...
	$('#wyca_by_step_mapping_zone_zoom_click').on('touchend', function(e) {
       e.preventDefault();
	   downOnZoomClickTrinary = false;
    });
	
	$('#wyca_by_step_mapping_zone_zoom_click').on('touchmove', function(e) {
       e.preventDefault();
	   if (downOnZoomClickTrinary)
	   {
		    w = $('#wyca_by_step_mapping_zoom_carte').width();
	...
	$('#wyca_by_step_mapping_svg').on('touchend', function(e) {
		resetPanTrinary = true;
	});
	$('#wyca_by_step_mapping_svg').on('touchstart', function(e) {
		resetPanTrinary = true;
	});
})
...
function InitTrinaryMap()
{
	...
	window.panZoomTrinary = svgPanZoom('#wyca_by_step_mapping_svg', {
	  zoomEnabled: true
	, controlIconsEnabled: false
	, maxZoom: 20
	, fit: 1
	...
	svgTrinary = document.querySelector('#wyca_by_step_mapping_svg .svg-pan-zoom_viewport');
...
{
	var obj = $('#wyca_by_step_mapping_svg g');
	obj.attr('id', 'wyca_by_step_mapping_svg_g');
	 var transformMatrix = obj.css("-webkit-transform") ||
	   obj.css("-moz-transform")    ||
	   obj.css("-ms-transform")     ||
	   obj.css("-o-transform")      ||
	...
	if (transformMatrix == undefined && typeof(window.panZoomTrinary) != 'undefined' )
	 	return  $('#wyca_by_step_mapping_image').attr('width') / $('#wyca_by_step_mapping_svg').width() / window.panZoomTrinary.getZoom()
...
function TrinaryRefreshZoomView()
{
	...
	pSVG = $('#wyca_by_step_mapping_svg').position();
	pImg = $('#wyca_by_step_mapping_svg image').position();
	pImg.left -= pSVG.left;
	pImg.top -= pSVG.top;
	
	//zoom = ros_width / $('#wyca_by_step_mapping_svg').width() / window.panZoom.getZoom();
	zoom = TrinaryGetZoom();
	
	wZoom = $('#wyca_by_step_mapping_zoom_carte').width();
	hZoom = $('#wyca_by_step_mapping_zoom_carte').height();
	...
	if (false && pImg.left > 0)
		...
		lNew = -(pImg.left*zoom) / $('#wyca_by_step_mapping_image').attr('width') * wZoom;
	if (false && pImg.top > 0)
		tNew = 0;
	else
		tNew = -(pImg.top*zoom) / $('#wyca_by_step_mapping_image').attr('width') * wZoom;
	
	hNew = $('#wyca_by_step_mapping_svg').height() * zoom  / $('#wyca_by_step_mapping_image').attr('width') * wZoom;
	wNew = $('#wyca_by_step_mapping_svg').width() * zoom  / $('#wyca_by_step_mapping_image').attr('width') * wZoom;
	
	//if (tNew + hNew > hZoom) hNew = hZoom - tNew;
	//if (lNew + wNew > wZoom) wNew = wZoom - lNew;
		
	$('#wyca_by_step_mapping_zone_zoom').width(wNew);
	$('#wyca_by_step_mapping_zone_zoom').height(hNew);
				
	$('#wyca_by_step_mapping_zone_zoom').css('top', tNew - 1);
	$('#wyca_by_step_mapping_zone_zoom').css('left', lNew - 1);
