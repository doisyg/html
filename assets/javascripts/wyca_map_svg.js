...
function WycaTraceArea(indexArea)
{
	...
	if (points.length > 1)
	{
		...
		if(!is_moving){
			...
			let svg_polygon = $('#wyca_edit_map_area_'+area.id_area);
			...
			$.each(points, function( indexPoint, point ) {
				let svg_line = $('#wyca_edit_map_svg #wyca_edit_map_area_trait_'+area.id_area+'_'+indexPoint);
				let svg_point = $('#wyca_edit_map_svg #wyca_edit_map_area_'+area.id_area+'_'+indexPoint);
...
function WycaTraceRobot(robot_x, robot_y, robot_theta)
{
	...
	else
	{
		if(robot_x == robot_y && robot_y == robot_theta && robot_x == 0 ){
			...
			$('#wyca_edit_map_robot_circle').hide();
			$('#wyca_edit_map_robot_sens').hide();
		}else{
			...
			$('#wyca_edit_map_tRobotNotLocalised').hide();
			$('#wyca_edit_map_robot_circle').show();
			$('#wyca_edit_map_robot_sens').show();
		}
	}
}
