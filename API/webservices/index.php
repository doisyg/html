...
if (!isset($userConnected))
{
	$app->group('/v1.0', function () use ($app){
		$app->group('/json', function () use ($app){
			$app->post('/connection', authenticate($app), function () use($app) {
			...
			$app->get('/map_contenu', authenticate($app), function () use($app) {
			...
			$app->get('/map', authenticate($app), function () use($app) {
			...
			$app->group('/robot', authenticate($app), function () use ($app){
