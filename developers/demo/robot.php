...
$actions = $userConnected->GetApiActions('groupe ASC, s.function_name', 'ASC');
...
<head>
	...
	<link href="<?php echo $_CONFIG['URL'];?>css/robot.css" rel="stylesheet">
    ...
    <script src="<?php echo $_CONFIG['URL_API'];?>wyca_api.latest.min.php?api_key=<?php echo $userConnected->api_key;?>"></script>
	...
	$(document).ready(function(e) {
		...
		wycaApi = new WycaAPI({
			...
			id_robot:'<?php echo $_GET['id_robot'];?>',
			video_element_id:'webcam_local',
			nick:'robot_<?php echo $_GET['id_robot'];?>',
			delay_no_reply : 30,
...
<?php
			...
			foreach($actions as $action)
			{
				...
				{
					?>
			on<?php echo $action->function_name;?>Feedback: function(data){
				if (data == undefined) data = 'ok';
				$('#on<?php echo $action->function_name;?>Feedback').html(JSON.stringify(data));
			},
			on<?php echo $action->function_name;?>Result: function(data){
				if (data == undefined) data = 'ok';
				$('#on<?php echo $action->function_name;?>Result').html(JSON.stringify(data));
			},
<?php
				}
			}
			?>
			onGetPhotoCamNav: function(data){
				$('#onGetPhotoCamNav').html('<img src="'+'data:image/jpeg;base64,'+ data + '" style="max-height:100px;" />');
			},
			onGetPhotoCamCustomer: function(data){
				$('#onGetPhotoCamCustomer').html('<img src="'+'data:image/jpeg;base64,'+ data + '" style="max-height:100px;" />');
			}
		});
...
<?php		
		...
		foreach($actions as $action)
		{
				...
				if (count($params['entree'])>0)
				{
					...
					foreach($params['entree'] as $param)
					{
						switch($param['type'])
						{
							...
							case 'int32':
							case 'uint8':
								$values[] = 'parseInt($(\'#input_'.$action->function_name.'_'.$param['nom'].'\').val())';
								break;
							case 'string':
								$values[] = '$(\'#input_'.$action->function_name.'_'.$param['nom'].'\').val()';
								break;
							case 'bool':
								$values[] = '$(\'#input_'.$action->function_name.'_'.$param['nom'].'\').val() == "true"';
								break;
							case 'geometry_msgs/Pose2D':
							
								$values[] = '{ x : parseFloat($(\'#input_'.$action->function_name.'_'.$param['nom'].'_x\').val()), y : parseFloat($(\'#input_'.$action->function_name.'_'.$param['nom'].'_y\').val()), theta : parseFloat($(\'#input_'.$action->function_name.'_'.$param['nom'].'_t\').val()) }';
								break;
							default:
								break;
						}
			...
			?>
		$('#form_<?php echo $action->function_name;?>').submit(function(e) {
            e.preventDefault();
			wycaApi.<?php echo $action->function_name;?>(<?php echo implode(', ', $values);?>);
			$('#modal_<?php echo $action->function_name;?>').modal('hide');
        });
<?php
				}
				else
				{
			?>
		$('#b<?php echo $action->function_name;?>').click(function(e) {
            e.preventDefault();
			wycaApi.<?php echo $action->function_name;?>();
        });
<?php		
				}
			}
		...
		$('.bHelp').click(function(e) {
			e.preventDefault();
            $(this).parent().children('.help').slideToggle();
        });
		
		$('#bTakePhotoBottom').click(function(e) {
            e.preventDefault();
			wycaApi.TakePhotoBottom();			
        });
		$('#bTakePhotoNav').click(function(e) {
            e.preventDefault();
			wycaApi.TakePhotoCamNav();			
        });
		$('#bTakePhotoCustomer').click(function(e) {
            e.preventDefault();
			wycaApi.TakePhotoCamCustomer();			
        });
	});
	...
	<style>
		video { width:640px !!important; height:320px !important; max-width:640px !important; }
	</style>
...
<body>
    ...
    <div id="webrtc" class="col-md-4">
        <div id="home">        
            ...
            <a href="#" id="bCall" class="btn btn-lg btn-primary">Call</a> <a href="#" class="bHelp"><i class="fa fa-question-circle"></i></a>
                <div class="help" style="position:absolute; display:none;">
                <pre><code>$('#bCall').click(function(e) {
    e.preventDefault();
        ...
        <div id="visio">
            <div id="videos">
                ...
                <div id="message">
                    ...
                    <div>
                    <input type="text" id="message_text" style="display:inline-block;" /><a href="#" id="bSendMessage" class="btn btn-xs btn-primary">Send</a> <a href="#" class="bHelp"><i class="fa fa-question-circle"></i></a>
                <div class="help" style="position:absolute; display:none;">
                <pre><code>$('#bSendMessage').click(function(e) {
	wycaApi.SendWebRTCMessage($('#message_text').val());
});
</code></pre>
</div>                
                    </div>
                    <h3>Messages received <a href="#" class="bHelp"><i class="fa fa-question-circle"></i></a>
                <div class="help" style="position:absolute; display:none;">
                <pre><code>onNewWebRTCMessage: function(data){
    $('#messages_recu').html($('#messages_recu').html() +'<br />' + data);
},
</code></pre>
</div> </h3>
    ...
    <div id="functions" class="col-md-4">
        <h2>Functions</h2>
        <h3>Photos</h3>
        <ul>
			<li><a href="#" id="bTakePhotoNav">Prendre photo cam nav</a>
            <img id="imgPhotoNav" src="" style="max-height:200px;" />
            </li>
			<li><a href="#" id="bTakePhotoCustomer">Prendre photo cam customer</a>
            <img id="imgPhotoCustomer" src="" style="max-height:200px;" /></li>
            
        </ul>
        <?php
		foreach($groupes as $groupe => $t)
		{
			if (isset($servicesParGroupe[$groupe]) || isset($actionsParGroupe[$groupe]))
			{
				?>
				<h3><?php echo $groupe;?></h3>
				<ul>
				<?php
				if (isset($servicesParGroupe[$groupe]))
				foreach($servicesParGroupe[$groupe] as $service)
				{
					if ($service->function_name != '')
					{
						...
						if (count($params['entree'])>0)
						{
					?><li><a href="#" data-toggle="modal" data-target="#modal_<?php echo $service->function_name;?>"><?php echo $service->function_name;?></a> <a href="#" class="bHelp"><i class="fa fa-question-circle"></i></a>
					<div class="help" style="position:absolute; display:none;">
					<pre><code><?php
						$params = $service->GetParams();
						$values = array();
					...
					?>wycaApi.<?php echo $service->function_name;?>(<?php echo implode(', ', $values);?>, function(reponse) { 
		<?php if ($service->function_name == 'MapGetHTML'){?>$('#map_top_display').html(reponse);<?php } else {?>$('#reponse_<?php echo $service->function_name;?>').html(JSON.stringify(reponse));<?php }?>
	});</code></pre>
					</div>
	...
	<?php }}?>
				...
				if (isset($actionsParGroupe[$groupe]))
				foreach($actionsParGroupe[$groupe] as $action)
				{
					if ($action->function_name != '')
					{
						...
						if (count($params['entree'])>0)
						{
					?><li><a href="#" data-toggle="modal" data-target="#modal_<?php echo $action->function_name;?>"><?php echo $action->function_name;?></a> <a href="#" class="bHelp"><i class="fa fa-question-circle"></i></a>
					<div class="help" style="position:absolute; display:none;">
					<pre><code><?php
						$params = $action->GetParams();
						$values = array();
					...
					?>wycaApi.<?php echo $action->function_name;?>(<?php echo implode(', ', $values);?>);</code></pre>
					</div>
					
					<span class="reponse" id="reponse_<?php echo $action->function_name;?>"></span></li>
					<?php
						}
						else
						{
					?><li><a href="#" id="b<?php echo $action->function_name;?>"><?php echo $action->function_name;?></a> <a href="#" class="bHelp"><i class="fa fa-question-circle"></i></a>
					<div class="help" style="position:absolute; display:none;">
					<pre><code>wycaApi.<?php echo $action->function_name;?>();</code></pre></div>
					<span class="reponse" id="reponse_<?php echo $action->function_name;?>"></span></li>
					<?php
						}
					}
    ...
    <div id="events" class="col-md-4">
		...
		foreach($groupes as $groupe => $t)
		{
			if (isset($topicsParGroupe[$groupe]) || isset($actionsParGroupe[$groupe]))
			{
				?>
				<h3><?php echo $groupe;?></h3>
				<ul>
				<?php
				if (isset($topicsParGroupe[$groupe]))
				...
				foreach($actionsParGroupe[$groupe] as $action)
				{
					if ($action->function_name != '')
					{
						?><li>on<?php echo $action->function_name;?>Feedback <span id="on<?php echo $action->function_name;?>Feedback"></span></li>
                        <li>on<?php echo $action->function_name;?>Result <span id="on<?php echo $action->function_name;?>Result"></span></li>
						<?php
					}
				}
				?>
...
foreach($actions as $action)
{
	if ($action->function_name != '')
	{
		...
		{
			?>
			<div class="modal fade" id="modal_<?php echo $action->function_name;?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
			  <div class="modal-dialog" role="document">
				<div class="modal-content">
				  <form id="form_<?php echo $action->function_name;?>" method="post" style="text-align:left;">
					  <div class="modal-header">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						  <span aria-hidden="true">&times;</span>
						</button>
					  ...
					  <div class="modal-body">
							...
							foreach($params['entree'] as $param)
							{
								?>
								<div class="form-group">
									<label for="input_<?php echo $action->function_name;?>_<?php echo $param['nom'];?>"><?php echo $param['nom'];?></label>
									<?php
                                    switch($param['type'])
                                    {
                                        case 'int8':
                                        case 'int32':
                                            ?><input type="number" name="input_<?php echo $action->function_name;?>_<?php echo $param['nom'];?>" id="input_<?php echo $action->function_name;?>_<?php echo $param['nom'];?>" class="form-control" /><?php
                                            break;
										case 'uint8':
                                            ?><input type="number" min="0" name="input_<?php echo $action->function_name;?>_<?php echo $param['nom'];?>" id="input_<?php echo $action->function_name;?>_<?php echo $param['nom'];?>" class="form-control" /><?php
                                            break;
										case 'string':
                                            ?><input type="text" name="input_<?php echo $action->function_name;?>_<?php echo $param['nom'];?>" id="input_<?php echo $action->function_name;?>_<?php echo $param['nom'];?>" class="form-control" /><?php
                                            break;
										case 'bool':
                                            ?><select name="input_<?php echo $action->function_name;?>_<?php echo $param['nom'];?>" id="input_<?php echo $action->function_name;?>_<?php echo $param['nom'];?>" class="form-control">
                                            	<option value="true">true</option>
                                            	<option value="false">false</option>
											</select><?php
                                            break;
										case 'geometry_msgs/Pose2D':
                                            ...
                                            <div class="input-group">
												<span class="input-group-addon" id="basic-addon1">x</span>
                                            	<input type="text" name="input_<?php echo $action->function_name;?>_<?php echo $param['nom'];?>" id="input_<?php echo $action->function_name;?>_<?php echo $param['nom'];?>_x" class="form-control" />
                                            </div>
                                            <div class="input-group">
												<span class="input-group-addon" id="basic-addon1">y</span>
											<input type="text" name="input_<?php echo $action->function_name;?>_<?php echo $param['nom'];?>" id="input_<?php echo $action->function_name;?>_<?php echo $param['nom'];?>_y" class="form-control" />
                                            </div>
                                            <div class="input-group">
												<span class="input-group-addon" id="basic-addon1">t</span>
											<input type="text" name="input_<?php echo $action->function_name;?>_<?php echo $param['nom'];?>" id="input_<?php echo $action->function_name;?>_<?php echo $param['nom'];?>_t" class="form-control" />
                                            </div><?php
											break;
                                        default:
                                            ?><script>console.log('Type <?php echo $param['type'];?> inconnu');</script><?php
