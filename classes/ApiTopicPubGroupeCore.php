...
class ApiTopicPubGroupeCore
{
	...
	public function __construct( $id_topic_pub_user = null, $byrow=false ) 
	{
		...
		{
			$object = ApiTopicPubGroupe::getApiTopicPubGroupe( $id_topic_pub_user );
			if ($object)
			{
				$this->Copy($object);
			}
	...
	public static function getApiTopicPubGroupe( $id_topic_pub_user )
	{
		$query = "SELECT * FROM api_topic_pub_groupe a1 WHERE id_topic_pub_user = '".mysqli_real_escape_string(DB::$connexion, $id_topic_pub_user)."'";
	...
	{
		$query="DELETE FROM api_topic_pub_groupe WHERE id_topic_pub_user = '".mysqli_real_escape_string(DB::$connexion, $this->id_topic_pub_user)."'";
